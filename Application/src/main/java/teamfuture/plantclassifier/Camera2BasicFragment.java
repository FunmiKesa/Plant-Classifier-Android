...
package teamfuture.plantclassifier;
...
//    public native String classify(long fileAddr);
    public native String classify(String fileAddr);
    ...
    private class SetUpNeuralNetwork extends AsyncTask<Void, Void, Void> {
        ...
        protected Void doInBackground(Void[] v) {
            try {
                ...
                toggleButton(false);
                initCaffe2(mgr);
                predictedClass = "Neural net loaded! Camera button is activated...";
    ...
    private void toggleButton(final boolean enable){
        ...
        if (activity != null){
            activity.runOnUiThread(new Runnable() {
                                       @Override
                                       public void run() {
                                           ((Button) activity.findViewById(R.id.picture)).setEnabled(enable);
    ...
    public void setText(String txt){
        ...
        if (activity != null) {
            activity.runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    TextView textView = (TextView) getView().findViewById(R.id.predictionText);
...
//            Bitmap resized_bitmap = Bitmap.createScaledBitmap(bitmap, 224, 224, false);
////            resized_bitmap.
            ...
            try {
                ...
                predictedClass = classify(mFile.getAbsolutePath());
                processing = false;
                setText(predictedClass);
